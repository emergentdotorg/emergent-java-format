...
import static org.emergent.javaformat.java.javadoc.JavadocLexer.LexException;
import static org.emergent.javaformat.java.javadoc.JavadocLexer.lex;
 ...
 *
 * <p>This stateless class reads tokens from the stateful lexer and translates them to "requests" and "writes" to the
 * stateful writer. It also munges tokens into "standardized" forms. Finally, it performs postprocessing to convert the
 * written Javadoc to a one-liner if possible or to leave a single blank line if it's empty.
 */
public final class JavadocFormatter {
    ...
    /**
     * Formats the given Javadoc comment, which must start with ∕✱✱ and end with ✱∕. The output will start and end with
     * the same characters.
     */
